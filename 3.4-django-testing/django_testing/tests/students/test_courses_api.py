...
def test_update_course(client, course_factory):
    ...
    assert reply['name'] == updated_course_name
...
@pytest.mark.django_db
def test_validate_students_fixture(student_factory):
    max_students = settings.MAX_STUDENTS_PER_COURSE
    students = student_factory(_quantity=max_students)
    assert len(students) == max_students


@pytest.fixture
def update_settings_fixture():
...
@pytest.mark.parametrize(['students_quantity', 'response_status_codes'],
                         ((1, [201]),                                                #успешний исход
                          (3, [201, 201, 201]),                                      #успешний исход
                          (5, [201, 201, 201, 201, 201]),                            #успешний исход
                          (10, [201, 201, 201, 201, 201, 400, 400, 400, 400, 400]),  #успешний исход
                          (10, [201, 201, 201, 201, 201, 201, 201, 201, 400, 400]))) #неудачный исход
@pytest.mark.django_db
def test_validate_students_quantity(client, course_factory, student_factory,
                                    update_settings_fixture, students_quantity, response_status_codes):
    course = course_factory(_quantity=1)
